...
async function launchChromeVisible(executablePath: string): Promise<{
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), 'octopus-browser-doctor-'));
  ...
  try {
    return await new Promise((resolve) => {
      ...
      const finish = (result: { ok: boolean; code?: number | null; signal?: NodeJS.Signals | null; stderr?: string; error?: string }) => {
        ...
        if (result.ok && child?.exitCode === null) child.kill('SIGTERM');
        resolve(result);
      };
    ...
    await stopDoctorChrome(child);
    await rm(userDataDir, { recursive: true, force: true }).catch(() => undefined);
  }
}

async function stopDoctorChrome(child: ChildProcess | undefined): Promise<void> {
  if (!child || child.exitCode !== null) return;
  child.kill('SIGTERM');
  await waitForDoctorChromeExit(child, 1500);
  if (child.exitCode !== null) return;
  child.kill('SIGKILL');
  await waitForDoctorChromeExit(child, 500);
}

async function waitForDoctorChromeExit(child: ChildProcess, timeoutMs: number): Promise<void> {
  if (child.exitCode !== null) return;
  await Promise.race([
