...
const SKIP_DETECT_CLOUD_SAVE_ENV = 'OCTOPUS_DETECT_SKIP_CLOUD_SAVE';
...
function assertAgentContext(value: DetectAgentContext): DetectAgentContext {
  if (value?.schemaVersion !== 'octopus.detect.agent-context.v1') throw new Error('无效的 Agent context schemaVersion。');
...
function validateRunSample(args: string[]): string | null {
  ...
  if (!raw || raw.startsWith('-')) return '--run-sample 需要一个正整数';
  return parseOptionalPositiveInt(raw) ? null : '--run-sample 需要一个正整数';
}
