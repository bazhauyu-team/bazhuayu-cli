    ...
    machineContract: {
      ...
      agentEntrypoint: {
        ...
        agentInvocationPolicy: {
          ...
          routingRule: 'If the user asks an LLM/agent to create, build, generate, or validate a scraping/collection task from a URL using bazhuayu-cli/octopus, call capabilities --json, then run the preferred recipe. Do not use detect --auto as the default agent path; --auto is only for direct CLI-only task generation when the user explicitly asks for automatic CLI selection. Use manual detect only when the user explicitly asks to choose elements themselves or when login/verification requires user action.'
        },
        intentAliases: [
