      ...
      recipes: {
        createTaskFromUrlWithAgent: {
          ...
          quickWorkflow: {
            ...
            command: 'octopus detect <url> --agent --agent-command <trusted-agent-runner> --goal <user task description> --output <task.json> --yes --run-sample 5 --json',
            ...
            notes: [
              ...
              'The trusted runner must open context.visualArtifacts.annotatedScreenshotPath or context.screenshot.path before writing OCTOPUS_AGENT_PLAN and include visualReview evidence/checks in the plan.',
          ...
          oneShotWrapper: {
            command: 'octopus detect <url> --agent --agent-command <cmd> --output <task.json> --yes [--run-sample <n>]',
            note: 'Use this only when a trusted agent runner command is available. --agent-command executes a local shell command. The runner receives OCTOPUS_AGENT_CONTEXT and must write OCTOPUS_AGENT_PLAN. --run-sample runs the generated task with --max-rows and embeds the run result in the same JSON envelope.'
          },
          nonGoals: [
