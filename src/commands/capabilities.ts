    ...
    machineContract: {
      ...
      agentEntrypoint: {
        ...
        agentInvocationPolicy: {
          ...
          routingRule: 'If the user asks an LLM/agent to create, build, generate, or validate a scraping/collection task from a URL using bazhuayu-cli/octopus, call capabilities --json, then run the preferred recipe. Use manual detect only when the user explicitly asks to choose elements themselves or when login/verification requires user action.'
        },
        intentAliases: [
      ...
      recipes: {
        createTaskFromUrlWithAgent: {
          ...
          searchWorkflow: {
            ...
            examples: [
              'octopus detect https://www.baidu.com/ --auto --query 李小龙 --output task.json',
          ...
          oneShotWrapper: {
            command: 'octopus detect <url> --agent --agent-command <cmd> --output <task.json>',
