...
export async function sendTaskControlCommand(taskId: string, command: RunControlCommand): Promise<RunControlState> {
  ...
  if (!state) {
    throw new Error(`任务 ${taskId} 没有正在运行的本地采集`);
  }
  const socketPath = resolveSocketPath(state.socketPath);
  if (!socketPath) {
    await cleanupStaleTaskState(state);
    throw new Error(`任务 ${taskId} 的本地控制通道不可用，已清理过期运行状态；请重新启动任务`);
  }
  ...
  if (!response.ok || !response.data) {
    throw new Error(response.error || `任务 ${taskId} 控制失败`);
  }
...
export async function sendRunControlCommand(outputDir: string, runId: string, command: RunControlCommand): Promise<RunControlState> {
  ...
  if (!state) {
    throw new Error(`运行 ${runId} 没有可用的本地控制通道；它可能已经结束，或 --output 不一致`);
  }
  const socketPath = resolveSocketPath(state.socketPath);
  if (!socketPath) {
    await cleanupStaleRunState(outputDir, runId, state);
    throw new Error(`运行 ${runId} 的控制通道不可用，已清理过期运行状态；请重新启动任务`);
  }
  ...
  if (!response.ok || !response.data) {
    throw new Error(response.error || `运行 ${runId} 控制失败`);
  }
  return response.data;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('end', () => {
      try {
        ...
        finish(new Error(`控制通道响应不是合法 JSON${raw ? `: ${raw.slice(0, 200)}` : ''}`));
      }
    });
    socket.on('close', () => {
      if (settled) return;
      if (!raw.trim()) {
        finish(new Error('控制通道无响应'));
        return;
      }
      try {
        finish(JSON.parse(raw.trim()) as RunControlResponse);
      } catch {
        finish(new Error(`控制通道响应不是合法 JSON: ${raw.slice(0, 200)}`));
      }
    });
  });
}
