...
export async function recognizePage(options: RecognizeOptions): Promise<PageRecognitionResult> {
  ...
  try {
    ...
    if (options.input && Object.keys(options.input).length) {
      ...
      const preDetectionLogin = await handleLoginIntervention('采集识别前检测到登录/验证要求');
...
async function waitForManualContinue(page: Page, targetUrl: string, runtimeConsole: SuppressedRuntimeConsole): Promise<ManualStartDecision> {
  while (true) {
    ...
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      runtimeConsole.writeStderr('如果网站跳转到登录页，请在浏览器中完成登录。看到要采集的数据后按 Enter 开始识别...\n');
    ...
    const response = await prompts({
      ...
      choices: [
        { title: '我已登录并看到要采集的数据，开始识别', value: 'recognize-logged-in' },
        { title: '当前页面不需要登录，开始识别', value: 'recognize-public' },
        { title: '重新打开目标页面', value: 'reload-target' },
        { title: '尝试关闭登录弹窗，不登录继续识别', value: 'dismiss-popups' },
...
async function handleLoginInterventionIfNeeded(host: ExtensionRecognizerHost, options: RecognizeOptions, runtimeConsole: SuppressedRuntimeConsole, reason: string): Promise<LoginInterventionResult> {
  ...
  try {
    ...
    const action = await chooseLoginInterventionInBrowser(host, message, initialAction, runtimeConsole)
      .catch(async () => {
        const response = await prompts({
          ...
          choices: [
            { title: '当前不需要登录，继续识别', value: 'continue-without-login' },
            { title: '我已完成登录/验证，继续识别', value: 'continue' },
            { title: '取消', value: 'cancel' }
          ],
    ...
    if (action === 'cancel' || !action) throw new Error('用户取消了登录后的识别');
    if (action === 'continue') {
      await adoptBestPageAfterLogin(host, options).catch(() => undefined);
      page = host.page;
    }
  ...
  const overlayOptions = {
    ...
    message: [
      ...
      initialAction === 'continue-without-login'
        ? '如果这只是普通登录弹窗，不影响当前页面内容，可以直接继续；本次识别后续将不再因同类弹窗打断。'
...
async function chooseCandidateInteractively(page: Page, candidates: RecognizedCandidate[], runtimeConsole: SuppressedRuntimeConsole): Promise<string[]> {
  ...
  try {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      runtimeConsole.writeStderr(`\n已在浏览器中标出推荐识别结果 1/${selectable.length}: ${formatCandidateSummary(currentCandidate)}。\n`);
    ...
    while (true) {
      const action = browserOverlayReady
        ...
        : await runLiveSelectMenu({
          write: (value) => runtimeConsole.writeStderr(value),
          title: () => [
            `当前识别结果: ${currentIndex + 1}/${selectable.length}`,
          ...
          choices: () => [
            { title: '确认当前识别结果，继续设置翻页', value: 'confirm' },
            { title: `切换到下一个识别结果 (${((currentIndex + 1) % selectable.length) + 1}/${selectable.length})`, value: 'next' },
            { title: `切换到上一个识别结果 (${((currentIndex - 1 + selectable.length) % selectable.length) + 1}/${selectable.length})`, value: 'prev' },
            { title: '取消手动识别', value: 'cancel' }
          ]
        });
      ...
      if (action === 'confirm') {
        ...
        if (browserOverlayReady) {
          await showManualProgressOverlay(page, {
            title: '正在分析翻页设置',
            message: '已确认识别结果，正在检测下一页、加载更多或滚动翻页。',
            status: '正在处理，请稍候。'
          }).then(() => {
...
async function showCandidateChoiceInBrowser(
  ...
  await showManualOverlay(page, {
    title: `识别结果 ${currentIndex + 1}/${selectable.length}`,
    message: [
      formatCandidateSummary(currentCandidate),
    ...
    choices: [
      { title: '确认当前识别结果', value: 'confirm', primary: true },
...
function formatCandidateSummary(candidate: RecognizedCandidate): string {
  ...
  return `${recognizerCandidateTypeLabel(candidate.type)}，${candidate.itemCount}条数据${fields ? `，字段: ${fields}` : ''}`;
}
...
async function chooseDetailPlanInteractively(page: Page, candidates: RecognizedCandidate[], runtimeConsole: SuppressedRuntimeConsole, timeoutMs: number): Promise<Map<string, RecognizedDetailPlan>> {
  ...
  for (const candidate of candidates) {
    ...
    const detail = await inspectDetailSampleManually(page, sampleUrls[0], runtimeConsole, timeoutMs).catch((error) => ({
      fields: [],
      sampleRows: [],
      reasons: [`详情页样例识别失败: ${error instanceof Error ? error.message : String(error)}`]
    }));
...
async function inspectDetailSamples(page: Page, urls: string[], timeoutMs: number): Promise<{ fields: RecognizedField[]; sampleRows: Record<string, string>[]; reasons: string[] }> {
  ...
  if (templateFields.length) reasons.push(`已从 ${rows.length} 个详情页样例识别字段: ${templateFields.map((field) => field.name).join(', ')}`);
...
async function runLiveSelectMenu<T extends string>(options: {
  ...
  const onData = (chunk: Buffer) => {
    ...
    if (value === '\u0003') {
      cleanup();
      rejectValue?.(new Error('用户取消了手动识别'));
      return;
    }
    if (value === '\u001b[A') {
  ...
  await page.evaluate((payload) => {
    ...
    for (const choice of choices) {
      ...
      button.addEventListener('click', (event) => {
        ...
        status.textContent = choice.value === 'cancel' ? '正在取消识别，请稍候。' : '已收到操作，正在处理，请稍候。';
        w.__octopusManualOverlayState = {
          ...(w.__octopusManualOverlayState || {}),
          action: choice.value
        };
...
function formatSelectedCandidates(ids: string[], candidates: RecognizedCandidate[]): string {
  ...
  return ids.map((id) => {
    ...
    return `${id} ${recognizerCandidateTypeLabel(candidate.type)} ${candidate.itemCount}条${fields ? ` [${fields}]` : ''}`;
  }).join('；');
}
...
async function chooseSearchInputRetryInCli(name: string, value: string): Promise<'retry' | 'cancel'> {
  const response = await prompts({
    ...
    choices: [
      ...
      { title: '取消搜索识别', description: '停止本次 recognize，避免选错输入框。', value: 'cancel' }
    ],
    initial: 0
  });
...
async function chooseSearchInputCandidateInCli(name: string, value: string, candidates: SearchInputCandidate[]): Promise<ManualOverlayAction> {
  ...
  choices.push({ title: '取消搜索识别', description: '停止本次 recognize，避免选错输入框。', value: 'cancel' });
  const response = await prompts({
    type: 'select',
    name: 'action',
