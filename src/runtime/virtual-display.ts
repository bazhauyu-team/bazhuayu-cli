...
export function virtualDisplayInstallMessage(): string {
  return 'Linux 服务器没有 DISPLAY/WAYLAND_DISPLAY，且当前浏览器运行时需要可见 Chrome。请安装系统浏览器依赖，例如 Ubuntu/Debian: apt-get update && apt-get install -y libnss3 libnspr4 xvfb，然后重试；手动检测也可以在桌面会话中运行。';
}
...
export async function startVirtualDisplayIfNeeded(): Promise<VirtualDisplayHandle> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'octopus-xvfb-'));
...
async function allocateDisplay(): Promise<string> {
  ...
  throw new Error('没有可用的 Xvfb display 端口。');
}
...
async function waitForDisplaySocket(display: string, child: ChildProcess): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null) {
      throw new Error(`Xvfb 启动失败: ${stderr.join('').trim() || `exit code ${child.exitCode}`}`);
    }
    await delay(50);
  }
  child.kill('SIGKILL');
  throw new Error(`Xvfb 启动超时: ${stderr.join('').trim()}`);
}
