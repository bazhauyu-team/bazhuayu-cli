 ...
 *
 * This module prepares task definitions for proprietary Bazhuayu / Octoparse
...
async function getRemoteTaskDefinition(taskId: string): Promise<TaskDefinition> {
  ...
  if (!auth.credential) {
    throw new Error(`Authentication required. Run "bazhuayu auth login" or set ${API_KEY_ENV}.`);
  }
...
async function taskNotFoundMessage(taskId: string, auth: Awaited<ReturnType<typeof resolveAuth>>['credential']): Promise<string> {
  ...
  if (!suggestion) {
    return `云端任务未找到或响应无效: ${taskId}。请用 "bazhuayu task list" 复制完整 taskId 后重试。`;
  }
  return [
    `云端任务未找到或响应无效: ${taskId}。`,
    `你是不是想运行: ${suggestion.taskId}${suggestion.taskName ? ` (${suggestion.taskName})` : ''}`,
    `命令: bazhuayu run ${suggestion.taskId}`
  ].join('\n');
}
...
function remoteTaskInfoToDefinition(info: RemoteTaskInfo, fallbackTaskId: string): TaskDefinition {
  ...
  if (!compressedXoml) {
    throw new Error(`云端任务 ${fallbackTaskId} 缺少 xoml`);
  }
...
function parseTextTaskFile(raw: string, taskId: string): Partial<TaskDefinition> {
  ...
  try {
    ...
    throw new Error(`taskFile 不是合法 JSON，也不是 XML: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function unzipTaskXoml(compressedXoml: string): string {
  ...
  if (buffer.byteLength <= 4) {
    throw new Error('OTD Xoml gzip 内容为空');
  }
...
function validateTask(task: TaskDefinition): void {
  if (!task.taskId) throw new Error('taskFile 缺少 taskId');
  if (task.apiList) {
    validateApiListTask(task);
    return;
  }
  if (!task.xml) throw new Error('taskFile 缺少 xml');
  if (!task.xoml) throw new Error('taskFile 缺少 xoml');
  if (!Array.isArray(task.fieldNames)) throw new Error('taskFile fieldNames 必须是数组');
  if (!/<process\b[^>]*isExecutable="true"/i.test(task.xoml)) {
    throw new Error('taskFile xoml 缺少可执行 BPMN process');
  }
  const actionTypes = extractActionTypes(task.xoml);
...
function validateApiListTask(task: TaskDefinition): void {
  ...
  if (!apiList || apiList.kind !== 'api_list') throw new Error('taskFile apiList.kind 必须是 api_list');
  if (!apiList.request?.url) throw new Error('taskFile apiList.request.url 不能为空');
  if (!/^https?:\/\//i.test(apiList.request.url)) throw new Error('taskFile apiList.request.url 必须是 http/https URL');
  if (apiList.request.method && apiList.request.method !== 'GET' && apiList.request.method !== 'POST') {
    throw new Error('taskFile apiList.request.method 只支持 GET 或 POST');
  }
  if (!apiList.itemsPath) throw new Error('taskFile apiList.itemsPath 不能为空');
  if (!Array.isArray(apiList.fields) || !apiList.fields.length) throw new Error('taskFile apiList.fields 必须是非空数组');
  for (const field of apiList.fields) {
    if (!field?.name || !field.path) throw new Error('taskFile apiList.fields 每项必须包含 name 和 path');
  }
}
