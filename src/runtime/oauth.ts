...
export const OAUTH_AUTHORITY_ENV = 'OCTOPUS_OAUTH_AUTHORITY';
export const OAUTH_CLIENT_ID_ENV = 'OCTOPUS_OAUTH_CLIENT_ID';
export const OAUTH_CLIENT_SECRET_ENV = 'OCTOPUS_OAUTH_CLIENT_SECRET';
export const OAUTH_REDIRECT_URI_ENV = 'OCTOPUS_OAUTH_REDIRECT_URI';
export const OAUTH_SCOPE_ENV = 'OCTOPUS_OAUTH_SCOPE';

export const DEFAULT_OAUTH_AUTHORITY = 'https://identity.bazhuayu.com';
export const DEFAULT_OAUTH_CLIENT_ID = 'bazhuayu-cli';
...
export async function runOAuthLogin(options: {
  ...
  const callback = new Promise<OAuthLoginResult>((resolveLogin, rejectLogin) => {
    const handler = async (request: IncomingMessage, response: ServerResponse<IncomingMessage>) => {
      try {
        ...
        if (error) {
          sendHtml(response, 400, renderStatusPage('登录失败', `${error}: ${url.searchParams.get('error_description') ?? ''}`));
        ...
        if (!returnedState || returnedState !== state) {
          sendHtml(response, 400, renderStatusPage('登录失败', 'state 校验失败，请重新登录。'));
        ...
        if (!code) {
          sendHtml(response, 400, renderStatusPage('登录失败', '回调缺少 authorization code。'));
        ...
        sendHtml(response, 200, renderStatusPage('登录成功', '可以关闭这个浏览器窗口，返回终端继续。'));
        resolveLogin({ token, config: activeConfig });
      } catch (error) {
        sendHtml(response, 500, renderStatusPage('登录失败', error instanceof Error ? error.message : String(error)));
        rejectLogin(error);
      }
    };
...
function normalizeExpiresInMs(expiresIn: number): number {
  // Bazhuayu Identity returns expires_in as a millisecond duration.
  // Keep second-based OAuth servers working for local tests or future compatibility.
