 ...
 *
 * This CLI may download, load, or invoke proprietary Bazhuayu / Octoparse
...
function loadPackageName(): string {
  ...
  return packageJson.name ?? 'bazhuayu-cli';
}
...
function loadReleaseNotesUrl(): string {
  ...
  const repositoryUrl = packageJson.repository?.url
    ?.replace(/^git\+/, '')
    .replace(/\.git$/, '');
  const baseUrl = repositoryUrl || packageJson.homepage?.replace(/#.*$/, '') || 'https://github.com/bazhauyu-team/bazhuayu-cli';
