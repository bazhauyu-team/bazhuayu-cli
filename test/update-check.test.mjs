...
test('maybePrintUpdateNotice prompts and updates when user chooses option 1', async () => {
  ...
  const fetchImpl = async (url) => {
    ...
    return new Response(JSON.stringify({ version: '0.1.16' }), {
      status: 200,
  ...
  assert.deepEqual(urls, ['https://registry.npmjs.org/bazhuayu-cli/latest']);
  assert.equal(prompts.length, 1);
  assert.match(prompts[0], /Update available! 0\.1\.15 -> 0\.1\.16/);
  assert.deepEqual(installs, ['bazhuayu-cli']);
  const cache = JSON.parse(await readFile(cacheFile, 'utf8'));
  assert.equal(cache.checkedAt, '2026-05-18T12:00:00.000Z');
  assert.equal(cache.latestVersion, '0.1.16');
});
...
test('maybePrintUpdateNotice does not fetch or print while cache is fresh', async () => {
  ...
  await maybePrintUpdateNotice({
    ...
    now: new Date('2026-05-18T12:00:00.000Z'),
    cacheFile,
    fetchImpl: async () => new Response(JSON.stringify({ version: '0.1.16' })),
    promptImpl: async () => '2',
    env: {},
    stdin: { isTTY: true },
