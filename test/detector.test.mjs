...
test('detectedTaskToCloudTaskInfo uses client-compatible compressed xml payload', async () => {
  const xml = '<ns0:RootAction useKernelBrowser="false"><x>标题</x></ns0:RootAction>';
...
test('manual overlay CLI hint is deduped by workflow stage across page changes', () => {
  ...
  writeManualOverlayHintOnceForTesting(runtimeConsole, { url: () => 'https://example.com/b' }, 'candidate', '确认检测结果\n');
...
test('buildTaskFromCandidate normalizes invalid element names in runtime XPath', () => {
  const task = buildTaskFromCandidate({
    ...
    candidate: {
      ...
      sampleRows: [{ area: '赤平市', url: 'https://example.com/area' }],
      reasons: ['test']
    }
  });
...
test('previewAgentPlanForTesting blocks sidebar candidates for primary content goals', () => {
  ...
  }, '采集当前页面主内容详情，包括标题、评分、地址和正文介绍，忽略侧栏推荐列表');
...
const context = JSON.parse(await readFile(process.env.OCTOPUS_AGENT_CONTEXT, 'utf8'));
await writeFile(${JSON.stringify(seenWorkDirFile)}, dirname(process.env.OCTOPUS_AGENT_CONTEXT));
const plan = {
  schemaVersion: 'octopus.detect.agent-plan.v1',
  visualReview: {
    reviewed: true,
...
await writeFile(process.env.OCTOPUS_AGENT_PLAN, JSON.stringify(plan, null, 2));
`);

  try {
    chdir(dir);
    process.env.OCTOPUS_API_KEY = 'detect-save-key';
...
import { readFile, writeFile } from 'node:fs/promises';
const context = JSON.parse(await readFile(process.env.OCTOPUS_AGENT_CONTEXT, 'utf8'));
await writeFile(process.env.OCTOPUS_AGENT_PLAN, JSON.stringify({
  schemaVersion: 'octopus.detect.agent-plan.v1',
  visualReview: {
    reviewed: true,
    screenshotPath: context.screenshot.path,
...
test('selectDetailUrlFieldForTesting accepts non-url href fields for detail prompts', () => {
  const candidate = {
    ...
    fields: [
      { name: 'title', kind: 'text', selector: 'a', xpath: '/html/body/main/article/a', relativeXPath: './a', samples: ['Alpha'] },
      { name: '标题链接', kind: 'href', selector: 'a', xpath: '/html/body/main/article/a', relativeXPath: './a', samples: ['https://example.com/a'] }
    ],
    sampleRows: [{ title: 'Alpha', '标题链接': 'https://example.com/a' }],
    reasons: ['test']
  };

  assert.equal(selectDetailUrlFieldForTesting(candidate)?.name, '标题链接');
});
...
test('selectDetailUrlFieldForTesting still prefers url over other href fields', () => {
  const candidate = {
    ...
    itemCount: 3,
    fields: [
      { name: '标题链接', kind: 'href', selector: 'a.title', xpath: '/html/body/main/article/a[1]', relativeXPath: './a[1]', samples: ['https://example.com/title'] },
      { name: 'url', kind: 'href', selector: 'a.detail', xpath: '/html/body/main/article/a[2]', relativeXPath: './a[2]', samples: ['https://example.com/detail'] }
    ],
    sampleRows: [{ '标题链接': 'https://example.com/title', url: 'https://example.com/detail' }],
    reasons: ['test']
  };
